...
regardless of who was signed in. Two symptoms traced to it: the hero "Open a
free tool" CTA and "Org sign-in" button stayed visible after login (surfaced
first by a super_admin session, redundant once the toolset is already on the
page), and the org-zone module tiles all read a static "– full" label
regardless of the tile's actual routing gate. Design session (POA-61) locked
the fix as a state model, not a copy patch: the page collapses to **two
states — anonymous vs. logged in** — for all copy/CTA/nav visibility. Tier
only affects the one tile with a real gate (BTC, `minTier: 'annual'`);
Throwdown/Liga Seduh/Cup Taster have no module-level gate and stay lit,
`Basic free` (anon) / `Full access` (logged in), no pill, in both states.

Design's recommended direction ("Command Console" — one adaptive bento
surface replacing the prior split of a marketing hero + separate dark org
zone) implemented in full, confirmed by Firdaus over the landing-in-place
alternative: the old two-zone layout is *why* POA-61's fix had to be applied
in two places instead of one, an anti-pattern already logged twice on this
project (POA-46, POA-51). `index.html` rewritten around a single
`[data-auth]`-driven console: one primary tool-launcher tile (same four
links for both states, labels/hints only differ), an account/login tile
reusing `shared/auth.js`'s existing `#org-email`/`#org-pass`/`data-act`
contract unchanged, and an always-visible module rail with BTC the only tile
driven by `Gates.canAccess('btc')` rather than role — verified correct for
super_admin without special-casing (a super_admin without an annual
subscription still sees the tile locked, "Annual plan").

Three of the design brief's own open questions (§6) were resolved with
Firdaus before implementation, since each changed real scope:
- **Personalised "Welcome back, {orgName}" greeting** — dropped for this
  pass. `shared/auth.js` only ever carries `user.email`, no org display
  name; adding one means a custom claim or a Firestore profile read, out of
  this ticket's `index.html`-only scope. Backlogged in `PLAN_OF_ACTION.md`
  under POA-61's entry. Shipped as a generic "Welcome back." instead.
- **Events reel + "Resume ▶" banner** (Console mockup's logged-in primary
  tile) — initially dropped rather than shipped as fake placeholder, since a
  non-functional Resume button implies working functionality with nothing
  behind it. **Revised later the same session** once real, honest options
  surfaced: the reel now mounts `shared/upcoming-events.js` (`media:
  'photo'`) — the same live feed the top ribbon already uses, not an
  independent copy — and Resume rows are generated by reading this device's
  own `shared/storage.js` saves directly (`seduh_throwdown_v1`,
  `seduh_liga_v1`, `seduh_cup_taster_v1`), linking straight into whichever
  module has a non-default `eventName` saved. This is honestly scoped to
  "what's saved in this browser" — there's still no cross-device event/
  session model — but it's the same scope every module already resumes into
  on its own when opened directly, so the front-page card isn't promising
  anything the platform doesn't already do. Renders an honest empty state
  ("No in-progress events saved on this device yet") when nothing's saved,
  verified via the local preview against real seeded `localStorage` state
  in both populated and empty cases.
  - **Follow-up (same session):** `shared/upcoming-events.js` gained a new
    `overlay: true` option on `media: 'photo'` mode — caption (kicker, name,
    date/venue, description, prev/next+counter) renders as an
    absolutely-positioned gradient-scrim overlay on the image instead of a
    separate content block stacked below it, matching the design mockup's
    reel treatment and saving vertical space in the bento tile. Purely
    additive and opt-in: `coming-soon/index.html`'s existing photo-mode call
    doesn't pass the option and is confirmed unchanged (verified: no
    `ue-overlay` class, `.ue-slide-content` stays `position: static`).
    `index.html`'s reel is the only consumer passing `overlay: true`.
- **Logged-in proof-band "what's new" item** — shipped as the mockup's
  placeholder copy ("Liga Seduh now exports CSV standings"), explicitly
  marked `TODO(POA-61)` in the markup — swap for the real changelog item
  before this ships live.

Also preserved unchanged, reused as-is inside the new layout: the
`shared/upcoming-events.js` featured-event ribbon, the admin-configured
photo slideshow (`initSlideshow()`, shown only when logged in and only if
slides exist, kept alongside the new reel/Resume tile per Firdaus — admin's
slideshow upload/reorder UI stays a live consumer, not orphaned), and the
module-info modal (README.md excerpt fetch, unchanged JS). `shared/gates.js`
not touched, reads only, per the design brief's scope lock.

**Dev tooling, same session:** `scripts/emulator-seed-events.js` added —
same dev-only pattern as the existing `emulator-seed-admin.js`/
`emulator-seed-org.js` (refuses to run without `FIRESTORE_EMULATOR_HOST`
set, never touches production). Writes three test docs into the Firestore
emulator's `upcoming_events` collection, matching the exact field shape
`admin/index.html` writes in production. Without it the emulator's Firestore
starts empty (no `--import` snapshot configured in `firebase.json`), so both
the ribbon and the new reel correctly — not buggily — showed "No upcoming
events yet" until seeded. Verified: ribbon and reel both render the real
seeded events after running it.

**Bug caught during verification, fixed before shipping:** an early pass of
the module-rail CSS copied a compound selector from the design mockup
(`.cs-doc:not([data-auth="in"])`) that assumed `[data-auth]` lived on the
same element as the `.cs-doc` wrapper class. In production `[data-auth]` is
set on `<html>` by `shared/auth.js`, not on a wrapping div, so the copied
selector always matched regardless of state and permanently dimmed the BTC
tile. Fixed to `html:not([data-auth="in"])`. Verified via the local preview
server (`npx serve`) with Firebase Emulator Suite: BTC tile dims correctly
across all four combinations (anon; logged in, not entitled; logged in,
entitled; back to anon), hero CTA/nav/tile visibility toggles correctly in
both states, module-info modal opens/closes, sign-in form retains the exact
DOM contract `shared/auth.js` depends on, and the layout is overflow-free
and reflows correctly at 375px mobile width.

`SEDUH_VERSION` bumped to `5.12.0`.
